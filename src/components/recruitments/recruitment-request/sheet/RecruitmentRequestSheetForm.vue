...
import { EDUCATION_LEVEL, listEmploymentType, listJobLevel } from '@/constants';
import type { IBranch, IDepartment, Job } from '@/types';
...
const listDepartment = computed(
	() =>
		(branches.value as IBranch[])?.map((item) => ({ label: item.name, value: item.id })) || [],
);
